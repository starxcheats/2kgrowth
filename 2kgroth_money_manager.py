...
Rules:
  - First trade of day = tier stake based on balance
  - On WIN  → continue; next stake = 50% of that profit (house money)
  - On LOSS → lock day immediately
  - No automatic lock after a win on any tier
"""
...
<style>
    ...
    .rules-box {
        background: #1a1f2e; border: 1px solid #2a3441; border-radius: 10px;
...
# =============================================================================
BASE_TIERS = [
    {"key": "emergency", "name": "Emergency Mode", "min_balance": 0.0,    "max_balance": 4.99,          "stake": 1.0,   "risk_pct": 25.0},
    {"key": "tier1",     "name": "Tier 1",         "min_balance": 5.0,    "max_balance": 29.99,         "stake": 5.0,   "risk_pct": 50.0},
    {"key": "tier2",     "name": "Tier 2",         "min_balance": 30.0,   "max_balance": 59.99,         "stake": 10.0,  "risk_pct": 33.3},
    {"key": "tier3",     "name": "Tier 3",         "min_balance": 60.0,   "max_balance": 99.99,         "stake": 20.0,  "risk_pct": 33.3},
    {"key": "tier4",     "name": "Tier 4",         "min_balance": 100.0,  "max_balance": 199.99,        "stake": 25.0,  "risk_pct": 25.0},
    {"key": "tier5",     "name": "Tier 5",         "min_balance": 200.0,  "max_balance": 499.99,        "stake": 30.0,  "risk_pct": 15.0},
    {"key": "tier6",     "name": "Tier 6",         "min_balance": 500.0,  "max_balance": float("inf"),  "stake": 100.0, "risk_pct": 20.0},
]
...
def build_tiers(scale: float) -> List[Dict]:
    ...
    for t in BASE_TIERS:
        ...
        nt["max_balance"] = float("inf") if t["max_balance"] == float("inf") else round(t["max_balance"] * scale, 2)
        nt["stake"] = round(t["stake"] * scale, 2)

        if t["key"] == "emergency":
            nt["name"] = "Emergency Mode"
            nt["description"] = f"Below ${5*scale:.0f} → stake ${nt['stake']:.0f} until ${10*scale:.0f} | Risk ~{t['risk_pct']}%"
        elif t["key"] == "tier1":
            nt["name"] = f"Tier 1 (${10*scale:.0f} Base)"
            nt["description"] = f"${nt['stake']:.0f}/trade · Risk {t['risk_pct']}%"
        elif t["key"] == "tier2":
...
def get_recommended_stake(balance: float, tiers: List[Dict]) -> Tuple[float, Dict, bool]:
    """
    - If house-money chain is active → use 50% of last profit
    - Otherwise → use tier stake for current balance
    """
...
def record_trade(result: str, pl_amount: Optional[float] = None):
    ...
    if st.session_state.day_locked:
        st.session_state.notifications.append({
            "type": "lock",
            "msg": "Trading is locked for today (loss already recorded). Return tomorrow.",
        })
        return
    ...
    # ========== CORE DISCIPLINE (as you requested) ==========
    if result == "WIN":
        # Keep trading — next stake = 50% of this profit
        ...
        st.session_state.notifications.append({
            ...
            "msg": (
                f"WIN (+${profit:.2f}). "
                f"Continue trading — next stake = 50% of profit → ${next_stake:.2f}. "
                f"Or press End Trading Day to stop."
            ),
        })
    else:
        # LOSS → lock the day
        ...
        st.session_state.notifications.append({
            ...
            "msg": (
                ...
                f"Day locked. No more trades until tomorrow."
            ),
        })
    ...
    if tier_after["name"] != tier_before["name"]:
        if balance_after < balance_before:
            st.session_state.notifications.append({
                "type": "stepdown",
                "msg": f"STEP-DOWN: {tier_before['name']} → {tier_after['name']}. Next tier stake = ${tier_after['stake']:.2f}.",
            })
        else:
            st.session_state.notifications.append({
                "type": "levelup",
                "msg": f"LEVEL-UP: {tier_before['name']} → {tier_after['name']}. Next tier stake = ${tier_after['stake']:.2f}.",
            })

    if balance_after < 5.0 * scale:
        st.session_state.notifications.append({
            "type": "stepdown",
            "msg": f"Emergency: below ${5*scale:.0f}. Stake = ${1*scale:.0f} until recover to ${10*scale:.0f}.",
        })
...
def compute_analytics(history: List[Dict], current_balance: float, start_balance: float) -> Dict:
    if not history:
        return {"total_days": 0, "net_roi": 0.0, "win_rate": 0.0, "current_streak": 0,
    ...
    return {
        "total_days": total_days, "net_roi": round(net_roi, 2), "win_rate": round(win_rate, 1),
        "current_streak": current_streak, "max_drawdown": round(max_dd, 2),
...
def build_equity_chart(equity_curve: List[Dict], scale: float) -> go.Figure:
    ...
    fig.add_trace(go.Scatter(
        ...
        line=dict(color="#00D4AA", width=2.5), marker=dict(size=6, color="#00D4AA"),
        fill="tozeroy", fillcolor="rgba(0, 212, 170, 0.08)",
    ))
    colors = ["#FFB800", "#FF8C00", "#FF6B6B", "#A855F7", "#3B82F6"]
    for i, m in enumerate(BASE_MILESTONES):
        scaled = round(m * scale, 2)
        fig.add_hline(y=scaled, line_dash="dot", line_color=colors[i % len(colors)],
    ...
    with st.sidebar:
        ...
        if st.session_state.mode == "Proportional Scaling Mode":
            custom = st.number_input(
                ...
                step=10.0,
                help="Scales ALL milestones & stakes from the $10 base.",
        ...
        st.markdown("---")
        st.markdown("### 📋 Core Tier Rules")
        st.markdown(f"""
<div class="rules-box">
<b>Stake by Balance (scale {scale:.0f}×)</b><br>
• &lt; ${5*scale:.0f} → <b>${1*scale:.0f}</b> Emergency<br>
• ${5*scale:.0f}–${30*scale-0.01:.0f} → <b>${5*scale:.0f}</b> Tier 1<br>
• ${30*scale:.0f}–${60*scale-0.01:.0f} → <b>${10*scale:.0f}</b> Tier 2<br>
• ${60*scale:.0f}–${100*scale-0.01:.0f} → <b>${20*scale:.0f}</b> Tier 3<br>
• ${100*scale:.0f}–${200*scale-0.01:.0f} → <b>${25*scale:.0f}</b> Tier 4<br>
• ${200*scale:.0f}–${500*scale-0.01:.0f} → <b>${30*scale:.0f}</b> Tier 5<br>
• ${500*scale:.0f}+ → <b>${100*scale:.0f}</b> Tier 6
</div>
""", unsafe_allow_html=True)

        st.markdown("""
<div class="rules-box">
<b>Discipline Rules</b><br>
• <span style="color:#00D4AA">WIN</span> → keep trading<br>
&nbsp;&nbsp;Next stake = <b>50% of that profit</b><br>
• <span style="color:#FF6B6B">LOSS</span> → <b>lock the day</b><br>
• First trade of day uses <b>tier stake</b>
</div>
""", unsafe_allow_html=True)
        ...
        st.markdown("---")
        st.caption("High-risk model. Only use capital you can afford to lose.")
    ...
    st.caption("WIN → continue at 50% profit · LOSS → lock day · Tier stake on first trade")
    ...
    for note in st.session_state.notifications[-4:]:
        ...
        icon = {"win": "✅", "loss": "❌", "lock": "🔒", "stepdown": "⬇️", "levelup": "⬆️", "house": "💰"}.get(ntype, "ℹ️")
    ...
    if st.session_state.day_locked:
        allowance = "Day Locked (Loss recorded)"
        allowance_color = "#FFB800"
    elif st.session_state.house_money_active:
        allowance = f"House Money Active · Next stake ${stake:.2f}"
        allowance_color = "#FFB800"
    else:
        allowance = "Ready — Tier stake for first trade"
    ...
    if is_house:
        house_badge = (
            ...
            'border-radius:4px;font-size:0.75rem;margin-left:8px;">HOUSE MONEY 50%</span>'
        )
    ...
    if not can_trade:
        st.markdown(
            '<div class="banner-lock">🔒 DAY LOCKED after loss. Return tomorrow.</div>',
            unsafe_allow_html=True,
        )
    elif st.session_state.house_money_active:
        st.markdown(
            f'<div class="banner-house">💰 Next trade at 50% of previous profit = <b>${stake:.2f}</b>. '
            f'Keep going or End Trading Day.</div>',
            unsafe_allow_html=True,
        )
    ...
    with col1:
        pl_input = st.number_input(
            ...
            value=0.0, step=0.01,
            help="Leave 0 = auto from stake × payout. + for win, − for loss.",
            disabled=not can_trade, key="pl_input",
        )
    with col2:
        st.write(""); st.write("")
        win_clicked = st.button("✅ RECORD WIN", use_container_width=True, type="primary",
                                disabled=not can_trade, key="btn_win")
    with col3:
        st.write(""); st.write("")
        loss_clicked = st.button("❌ RECORD LOSS", use_container_width=True,
                                 disabled=not can_trade, key="btn_loss")

    end_col1, end_col2, end_col3 = st.columns([1, 1, 1])
    with end_col2:
        if st.button("⏹️ End Trading Day & Lock Terminal", use_container_width=True,
                     disabled=st.session_state.day_locked, key="btn_end"):
            end_trading_day()
            st.rerun()
    ...
    if win_clicked:
        ...
        if pl is not None and pl < 0:
            st.error("For a WIN, P/L must be positive (or leave 0).")
        else:
    ...
    if loss_clicked:
        pl = pl_input if pl_input != 0 else None
        if pl is not None and pl > 0:
            st.error("For a LOSS, P/L must be negative (or leave 0).")
        else:
    ...
    st.markdown(
        ...
        "⚠️ <b>HIGH-RISK WARNING</b>: 15%–50% single-trade risk. "
    ...
    metrics = compute_analytics(st.session_state.trade_history, st.session_state.balance, original_start)
    ...
    if st.session_state.trade_history:
        ...
        display_df.columns = ["Date", "Tier", "Start Bal", "Stake", "Result", "P/L", "House $", "End Bal"]
    ...
    st.markdown("---")
    st.caption("WIN → continue at 50% profit · LOSS → lock day · Tier stake on first trade of day")
